...
> [!NOTE]
> **分光結晶**や**測定法**によって、コード内の数値・文字列を変える必要があります。
...
**分光結晶** : 実測値を記入してください。通常、**Si(111) = 3.13551** (デフォルト)、Si(311) = 1.63747 です。

**測定法** : 従来の**透過法ならば math.log(float(values[1]) / float(values[2]))** (デフォルト)、蛍光法ならば float(values[2]) / float(values[1])
